...
def dice_similarity(u: numpy.ndarray, v: numpy.ndarray) -> numpy.float64:
    ...
    """
    u_abs_and_v_abs = (u+v).sum() # numpy.abs(u).sum() + numpy.abs(v).sum()
    dice_score = 0
    if u_abs_and_v_abs != 0:
        #u_and_v = (numpy.bitwise_and(u != 0, v != 0)).sum()
        u_and_v = (u*v).sum()
        dice_score = 2.0 * numpy.float64(u_and_v) / numpy.float64(u_abs_and_v_abs)
    return dice_score
